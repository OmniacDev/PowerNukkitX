...
public class ChestInventory extends ContainerInventory implements BlockEntityInventoryNameable {
    ...
    public void onOpen(Player who) {
        ...
        if (this.getViewers().size() == 1) {
            ...
            if (level != null) {
                level.addSound(this.getHolder().position.add(0.5, 0.5, 0.5), Sound.RANDOM_CHESTOPEN);
    ...
    public void onClose(Player who) {
        if (this.getViewers().size() == 1) {
            ...
            Level level = this.getHolder().getLevel();
            if (level != null) {
                level.addSound(this.getHolder().position.add(0.5, 0.5, 0.5), Sound.RANDOM_CHESTCLOSED);
                level.addChunkPacket((int) this.getHolder().getX() >> 4, (int) this.getHolder().getZ() >> 4, pk);
            }
        }
