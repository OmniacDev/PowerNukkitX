 ...
 * @author PikyCZ
 */
public class EntityElderGuardian extends EntityMonster implements EntitySwimmable {
        ...
        return new BehaviorGroup(
                ...
                Set.of(
                        ...
                        new Behavior(new GuardianAttackExecutor(CoreMemoryTypes.NEAREST_PLAYER, 0.3f, 15, true, 60, 40), all(
                                ...
                                entity -> entity.getMemoryStorage().get(CoreMemoryTypes.NEAREST_PLAYER) != null && this.level.raycastBlocks(entity.position, entity.getMemoryStorage().get(CoreMemoryTypes.NEAREST_PLAYER).position).stream().allMatch(Block::isTransparent)
                        ), 3, 1),
    ...
    public boolean onUpdate(int currentTick) {
        if (!this.closed && this.isAlive()) {
            for (Player p : this.getViewers().values()) {
                if (p.locallyInitialized && p.getGamemode() % 2 == 0 && p.position.distance(this.position) < 50 && !p.hasEffect(EffectType.MINING_FATIGUE)) {
