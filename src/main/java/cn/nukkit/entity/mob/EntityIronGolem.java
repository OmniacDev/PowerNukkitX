...
public class EntityIronGolem extends EntityGolem implements EntityOwnable {
    ...
    public IBehaviorGroup requireBehaviorGroup() {
        return new BehaviorGroup(
                ...
                Set.of(new NearestEntitySensor(EntityMonster.class, CoreMemoryTypes.NEAREST_SHARED_ENTITY, 16, 0)),
    ...
    public boolean onInteract(Player player, Item item, Vector3 clickedPos) {
        if(item instanceof ItemIronIngot && getHealth() <= getMaxHealth()*0.75f) {
            this.level.addSound(this.position, Sound.MOB_IRONGOLEM_REPAIR);
    ...
    public boolean attack(EntityDamageEvent source) {
        ...
        for(int i : new int[] {74, 50, 25}) {
            if(health > i && getHealth() <= i) {
                this.level.addSound(this.position, Sound.MOB_IRONGOLEM_CRACK);
            }
        }
        return true;
    }
            ...
            if(block instanceof BlockPumpkin) {
                ...
                for(BlockFace blockFace : BlockFace.values()) {
                    ...
                    for(BlockFace face : Set.of(BlockFace.UP, BlockFace.NORTH, BlockFace.EAST)) {
                        ...
                        Entity irongolem = Entity.createEntity(EntityID.IRON_GOLEM, block.level.getChunk(block.position.getChunkX(), block.position.getChunkZ()), nbt);
