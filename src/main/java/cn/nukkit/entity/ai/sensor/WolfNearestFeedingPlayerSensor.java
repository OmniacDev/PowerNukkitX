    ...
    public void sense(EntityMob entity) {
        if (entity instanceof EntityAnimal entityAnimal) {
            ...
            for (Player p : entity.level.getPlayers().values()) {
                if (entity.position.distanceSquared(p.position) <= rangeSquared && entity.position.distanceSquared(p.position) >= minRangeSquared && (p.getInventory().getItemInHand().getId() == Item.BONE || entityAnimal.isBreedingItem(p.getInventory().getItemInHand()))) {
                    if (player == null) {
                        player = p;
                    } else {
