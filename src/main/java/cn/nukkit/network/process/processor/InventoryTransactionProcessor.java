        ...
        switch (type) {
            case InventoryTransactionPacket.USE_ITEM_ON_ENTITY_ACTION_INTERACT -> {
                ...
                if (!(target instanceof EntityArmorStand)) {
                    player.level.getVibrationManager().callVibrationEvent(new VibrationEvent(target, target.position.clone(), VibrationType.ENTITY_INTERACT));
                } else {
                    player.level.getVibrationManager().callVibrationEvent(new VibrationEvent(target, target.position.clone(), VibrationType.EQUIP));
                }
            ...
            case InventoryTransactionPacket.USE_ITEM_ON_ENTITY_ACTION_ATTACK -> {
                ...
                if (!player.canInteract(target.position, player.isCreative() ? 8 : 5)) {
                    return;
