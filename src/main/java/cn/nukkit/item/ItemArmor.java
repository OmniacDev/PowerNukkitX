...
abstract public class ItemArmor extends Item implements ItemDurable {
    ...
    public boolean onClickAir(Player player, Vector3 directionVector) {
        ...
        if (equip) {
            ...
            switch (tier) {
                case TIER_CHAIN:
                    player.getLevel().addSound(player.position, Sound.ARMOR_EQUIP_CHAIN);
                    break;
                case TIER_DIAMOND:
                    player.getLevel().addSound(player.position, Sound.ARMOR_EQUIP_DIAMOND);
                    break;
                case TIER_GOLD:
                    player.getLevel().addSound(player.position, Sound.ARMOR_EQUIP_GOLD);
                    break;
                case TIER_IRON:
                    player.getLevel().addSound(player.position, Sound.ARMOR_EQUIP_IRON);
                    break;
                case TIER_LEATHER:
                    player.getLevel().addSound(player.position, Sound.ARMOR_EQUIP_LEATHER);
                    break;
                case TIER_NETHERITE:
                    player.getLevel().addSound(player.position, Sound.ARMOR_EQUIP_NETHERITE);
                    break;
                default:
                    player.getLevel().addSound(player.position, Sound.ARMOR_EQUIP_GENERIC);
            }
        }
