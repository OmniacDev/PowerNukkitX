...
//    @NotNull public Integer timeSinceRest = 0;
    ...
    protected void onBlockBreakContinue(Vector3 pos, BlockFace face) {
        if (this.isBreakingBlock()) {
            ...
            if (miningTimeRequired > 0) {
                ...
                pk.x = (float) this.breakingBlock.position.x;
    ...
    protected void onBlockBreakStart(Vector3 pos, BlockFace face) {
        ...
        if ((this.lastBreakPosition.equals(blockPos) && (currentBreak - this.lastBreak) < 10) || pos.distanceSquared(this.position) > 1000) {
            return;
        }

        Block target = this.level.getBlock(pos);
        PlayerInteractEvent playerInteractEvent = new PlayerInteractEvent(this, this.inventory.getItemInHand(), target.position, face,
        ...
        if (block.getId().equals(BlockID.SWEET_BERRY_BUSH) && block.isDefaultState()) {
            ...
            Item i = this.level.useBreakOn(block.position, oldItem, this, true);
    ...
    protected void initEntity() {
        ...
        if (this.namedTag.containsInt("SpawnX") && this.namedTag.containsInt("SpawnY") && this.namedTag.containsInt("SpawnZ")) {
            this.spawnPoint = new Locator(this.namedTag.getInt("SpawnX"), this.namedTag.getInt("SpawnY"), this.namedTag.getInt("SpawnZ"), level);
        } else {
            this.spawnPoint = level.getSafeSpawn();
            log.info("Player {} cannot find the saved spawnpoint, reset the spawnpoint to {} {} {} / {}", this.getName(), this.spawnPoint.position.x, this.spawnPoint.position.y, this.spawnPoint.position.z, this.spawnPoint.getLevel().getName());
        }
    ...
    protected void doFirstSpawn() {
        ...
        if (this.getSpawn().second() == null || this.getSpawn().second() == SpawnPointType.WORLD) {
            ...
            pk.x = this.getSpawn().first().position.getFloorX();
            pk.y = this.getSpawn().first().position.getFloorY();
            pk.z = this.getSpawn().first().position.getFloorZ();
    ...
    protected void checkGroundState(double movX, double movY, double movZ, double dx, double dy, double dz) {
        if (!this.onGround || movX != 0 || movY != 0 || movZ != 0) {
            ...
            Block b1 = level.getTickCachedBlock(this.position.getFloorX(), this.position.getFloorY() - 1, this.position.getFloorZ());
            Block b2 = level.getTickCachedBlock(this.position.getFloorX(), this.position.getFloorY(), this.position.getFloorZ());
            Block[] blocks = {
                    level.getTickCachedBlock(this.position.getFloorX() - 1, this.position.getFloorY() - 1, this.position.getFloorZ()),
                    level.getTickCachedBlock(this.position.getFloorX() + 1, this.position.getFloorY() - 1, this.position.getFloorZ()),
                    level.getTickCachedBlock(this.position.getFloorX(), this.position.getFloorY() - 1, this.position.getFloorZ() + 1),
                    level.getTickCachedBlock(this.position.getFloorX(), this.position.getFloorY() - 1, this.position.getFloorZ() - 1),
                    level.getTickCachedBlock(this.position.getFloorX() - 1, this.position.getFloorY() - 1, this.position.getFloorZ() - 1),
                    level.getTickCachedBlock(this.position.getFloorX() + 1, this.position.getFloorY() - 1, this.position.getFloorZ() - 1),
                    level.getTickCachedBlock(this.position.getFloorX() + 1, this.position.getFloorY() - 1, this.position.getFloorZ() + 1),
                    level.getTickCachedBlock(this.position.getFloorX() - 1, this.position.getFloorY() - 1, this.position.getFloorZ() + 1)
            };
...
//                level.addParticle(new BlockForceFieldParticle(block.add(0.5, 0, 0.5)));
    ...
    protected void handleMovement(Transform clientPos) {
        ...
        double distance = clientPos.position.distanceSquared(this.position);
        //before check
        if (distance > 128) {
            invalidMotion = true;
        ...
        double diffX = clientPos.getX() - this.position.x;
        double diffY = clientPos.getY() - this.position.y;
        double diffZ = clientPos.getZ() - this.position.z;
        this.setRotation(clientPos.getYaw(), clientPos.getPitch(), clientPos.getHeadYaw());
        this.fastMove(diffX, diffY, diffZ);

        //after check
        double corrX = this.position.x - clientPos.getX();
        double corrY = this.position.y - clientPos.getY();
        double corrZ = this.position.z - clientPos.getZ();
        ...
        //update speed
        if (this.speed == null) {
            this.speed = new Vector3(last.position.x - now.position.x, last.position.y - now.position.y, last.position.z - now.position.z);
        } else {
            this.speed.setComponents(last.position.x - now.position.x, last.position.y - now.position.y, last.position.z - now.position.z);
        }
    ...
    protected void offerMovementTask(Transform newPosition) {
        var distance = newPosition.position.distance(this.position);
    ...
    protected void handleLogicInMove(boolean invalidMotion, double distance) {
        if (!invalidMotion) {
            ...
            if (frostWalker != null && frostWalker.getLevel() > 0 && !this.isSpectator() && this.position.y >= 1 && this.position.y <= 255) {
                int radius = 2 + frostWalker.getLevel();
                for (int coordX = this.position.getFloorX() - radius; coordX < this.position.getFloorX() + radius + 1; coordX++) {
                    for (int coordZ = this.position.getFloorZ() - radius; coordZ < this.position.getFloorZ() + radius + 1; coordZ++) {
                        Block block = level.getBlock(coordX, this.position.getFloorY() - 1, coordZ);
                        int layer = 0;
                        ...
                        if (!ev.isCancelled()) {
                            ...
                            level.scheduleUpdate(level.getBlock(block.position, layer), ThreadLocalRandom.current().nextInt(20, 40));
                        }
                    }
                }
            }
    ...
    public void processLogin() {
        ...
        if ((level = this.server.getLevelByName(nbt.getString("Level"))) == null) {
            ...
            nbt.getList("Pos", FloatTag.class)
                    .add(new FloatTag(spawnLocation.position.x))
        ...
        log.info(server.getLanguage().tr("nukkit.player.logIn",
                ...
                this.level.getName(),
                String.valueOf(NukkitMath.round(this.position.x, 4)),
                String.valueOf(NukkitMath.round(this.position.y, 4)),
                String.valueOf(NukkitMath.round(this.position.z, 4))));
    }
    ...
    protected void respawn() {
        ...
        Pair<Locator, SpawnPointType> spawnPair = this.getSpawn();
            ...
            if (spawnBlock != null && isValidRespawnBlock(spawnBlock)) {
                ...
                if (spawnBlock.getId().equals(BlockID.RESPAWN_ANCHOR)) {
                    ...
                    if (charge > 0) {
                        ...
                        respawnAnchor.getLevel().setBlock(respawnAnchor.position, spawnBlock);
                        respawnAnchor.getLevel().scheduleUpdate(respawnAnchor, 10);
                        respawnAnchor.getLevel().addSound(this.position, Sound.RESPAWN_ANCHOR_DEPLETE, 1, 1, this);
                    }
                }
            } else {//block not available
                Locator defaultSpawn = this.getServer().getDefaultLevel().getSpawnLocation();
        ...
        Locator respawnPos = playerRespawnEvent.getRespawnPosition().first();
    ...
    protected void forceSendEmptyChunks() {
        int chunkPositionX = this.position.getFloorX() >> 4;
        int chunkPositionZ = this.position.getFloorZ() >> 4;
...
//            this.server.updatePlayerListData(this.getUniqueId(), this.getId(), this.getDisplayName(), skin, this.getLoginChainData().getXUID());
     ...
     * @return {@link Locator}
     */
    public Pair<Locator, SpawnPointType> getSpawn() {
     ...
     * @param pos 出生点位置
     */
    public void setSpawn(Locator pos, SpawnPointType spawnPointType) {
        Preconditions.checkNotNull(pos);
        Preconditions.checkNotNull(pos.level);
        this.spawnPoint = new Locator(pos.position.x, pos.position.y, pos.position.z, level);
        this.spawnPointType = spawnPointType;
        SetSpawnPositionPacket pk = new SetSpawnPositionPacket();
        pk.spawnType = SetSpawnPositionPacket.TYPE_PLAYER_SPAWN;
        pk.x = (int) this.spawnPoint.position.x;
        pk.y = (int) this.spawnPoint.position.y;
        pk.z = (int) this.spawnPoint.position.z;
    ...
    public boolean sleepOn(Vector3 pos) {
        ...
        this.teleport(new Transform(pos.x + 0.5, pos.y + 0.5, pos.z + 0.5, this.rotation.yaw, this.rotation.pitch, this.level), null);
    ...
    public AxisAlignedBB reCalcOffsetBoundingBox() {
        ...
        return this.offsetBoundingBox.setBounds(
                this.position.x - dx, this.position.y, this.position.z - dz,
                this.position.x + dx, this.position.y + this.getHeight(), this.position.z + dz
        );
    }

    @Override
    public void moveDelta() {
        this.sendPosition(this.position, this.rotation.yaw, this.rotation.pitch, MovePlayerPacket.MODE_NORMAL, this.getViewers().values().toArray(EMPTY_ARRAY));
    }
    ...
    public void setSwimming(boolean value) {
        ...
        if (!value && level.getBlock(this.position.up()).isSolid() && level.getBlock(this.position.down()).isSolid()) {
            return;
        }
    ...
    public boolean onUpdate(int currentTick) {
        ...
        if (this.fishing != null && this.level.getTick() % 20 == 0) {
            if (this.position.distance(fishing.position) > 33) {
            ...
            if (!this.isSpectator() && this.speed != null) {
                if (this.onGround) {
                    ...
                    if (this.checkMovement && !this.isGliding() && !server.getAllowFlight() &&
                        ...
                        Block block = level.getBlock(this.position);
                    ...
                    if (this.position.y > highestPosition) {
                        this.highestPosition = this.position.y;
                    }
    ...
    public void checkInteractNearby() {
        ...
        if (canInteract(this.position, interactDistance)) {
    ...
    private EntityInteractable getEntityAtPosition(Entity[] nearbyEntities, int x, int y, int z) {
        for (Entity nearestEntity : nearbyEntities) {
            if (nearestEntity.position.getFloorX() == x && nearestEntity.position.getFloorY() == y && nearestEntity.position.getFloorZ() == z
    ...
    public EntityInteractable getEntityPlayerLookingAt(int maxDistance) {
        ...
        try {
            ...
            if (itr.hasNext()) {
                ...
                while (itr.hasNext()) {
                    block = itr.next();
                    entity = getEntityAtPosition(nearbyEntities, block.position.getFloorX(), block.position.getFloorY(), block.position.getFloorZ());
                    if (entity != null) {
                        break;
                    }
                }
    ...
    public boolean canInteract(Vector3 pos, double maxDistance, double maxDiff) {
        if (this.position.distanceSquared(pos) > maxDistance * maxDistance) {
            return false;
        }

        Vector2 dV = this.getDirectionPlane();
        double dot = dV.dot(new Vector2(this.position.x, this.position.z));
    ...
    public void saveNBT() {
        ...
        if (spawnPoint == null) {
            ...
            namedTag.putInt("SpawnX", this.spawnPoint.position.getFloorX())
                    .putInt("SpawnY", this.spawnPoint.position.getFloorY())
                    .putInt("SpawnZ", this.spawnPoint.position.getFloorZ());
    ...
    public void kill() {
        ...
        if (!ev.isCancelled()) {
            ...
            if (!ev.getKeepInventory() && this.level.getGameRules().getBoolean(GameRule.DO_ENTITY_DROPS)) {
                for (Item item : ev.getDrops()) {
                    if (!item.hasEnchantment(Enchantment.ID_VANISHING_CURSE) && item.applyEnchantments()) {
                        this.level.dropItem(this.position, item, null, true, 40);
                    }
                }
            ...
            this.setDataProperty(PLAYER_LAST_DEATH_POS, new BlockVector3(this.position.getFloorX(), this.position.getFloorY(), this.position.getFloorZ()));

            RespawnPacket pk = new RespawnPacket();
            Locator pos = this.getSpawn().left();
            pk.x = (float) pos.position.x;
            pk.y = (float) pos.position.y;
            pk.z = (float) pos.position.z;
    ...
    public boolean dropItem(Item item) {
        ...
        this.level.dropItem(this.position.add(0, 1.3, 0), item, motion, 40);
    ...
    public @Nullable EntityItem dropAndGetItem(@NotNull Item item) {
        ...
        return this.level.dropAndGetItem(this.position.add(0, 1.3, 0), item, motion, 40);
    }
    ...
    @Override
    public boolean teleport(Transform transform, TeleportCause cause) {
        if (!this.isOnline()) {
            return false;
        }
        ...
        getLevel().getScheduler().scheduleDelayedTask(InternalPlugin.INSTANCE, () -> {
            for (var b : this.level.getBlockEntities().values()) {
                ...
                if (b instanceof BlockEntitySpawnable blockEntitySpawnable) {
                    ...
                    setAir.x = b.position.getFloorX();
                    setAir.y = b.position.getFloorY();
                    setAir.z = b.position.getFloorZ();
                    ...
                    revertAir.x = b.position.getFloorX();
                    revertAir.y = b.position.getFloorY();
                    revertAir.z = b.position.getFloorZ();
    ...
    public boolean switchLevel(Level level) {
        if (super.switchLevel(level)) {
            ...
            Locator spawn = level.getSpawnLocation();
            spawnPosition.x = spawn.position.getFloorX();
            spawnPosition.y = spawn.position.getFloorY();
            spawnPosition.z = spawn.position.getFloorZ();
    ...
    public void startFishing(Item fishingRod) {
        CompoundTag nbt = new CompoundTag()
                .putList("Pos", new ListTag<FloatTag>()
                        ...
                        .add(new FloatTag(this.position.y + this.getEyeHeight()))
