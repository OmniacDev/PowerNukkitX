...
public abstract class BlockVinesNether extends BlockTransparent {
    ...
    @NotNull public Optional<BlockVinesNether> findVineBlock(boolean base) {
        return findVine(base)
                .map(LevelPosition::getLevelBlock)
     ...
     */
    @NotNull public Optional<LevelPosition> findVine(boolean base) {
        BlockFace supportFace = getGrowthDirection();
        if (base) {
            supportFace = supportFace.getOpposite();
        }
        LevelPosition result = getLocation();
        String id = getId();
        int limit = 256;
        while (--limit > 0){
            LevelPosition next = result.getSide(supportFace);
    ...
    @NotNull public OptionalBoolean increaseRootAge() {
        Block base = findVine(true).map(LevelPosition::getLevelBlock).orElse(null);
