...
public class BlockSuspiciousSand extends BlockFallable {
    ...
    public void onTouch(@NotNull Vector3 vector, @NotNull Item item, @NotNull BlockFace face, float fx, float fy, float fz, @Nullable Player player, PlayerInteractEvent.@NotNull Action action) {
        ...
        if(progress < 3) {
            ...
            getLevel().addSound(this.position, Sound.HIT_SUSPICIOUS_SAND);
            getLevel().setBlock(this.position, this);
        } else {
            getLevel().setBlock(this.position, BlockSand.PROPERTIES.getDefaultState().toBlock());
            getLevel().addSound(this.position, Sound.BREAK_SUSPICIOUS_SAND);
        }
