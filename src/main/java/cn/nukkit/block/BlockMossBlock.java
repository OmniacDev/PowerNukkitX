...
public class BlockMossBlock extends BlockSolid implements Natural {
    ...
    public boolean canBePopulated(LevelPosition pos) {
    ...
    public boolean canBePopulated2BlockAir(LevelPosition pos) {
    ...
    public void convertToMoss(LevelPosition pos) {
        Random random = new Random();
        for (double x = pos.x - 3; x <= pos.x + 3; x++) {
            for (double z = pos.z - 3; z <= pos.z + 3; z++) {
                for (double y = pos.y + 5; y >= pos.y - 5; y--) {
                    if (canConvertToMoss(pos.level.getBlock(new LevelPosition(x, y, z, pos.level))) && (random.nextDouble() < 0.6 || Math.abs(x - pos.x) < 3 && Math.abs(z - pos.z) < 3)) {
                        pos.level.setBlock(new LevelPosition(x, y, z, pos.level), Block.get(BlockID.MOSS_BLOCK));
                        break;
                    }
                }
            }
    ...
    public void populateRegion(LevelPosition pos) {
        Random random = new Random();
        for (double x = pos.x - 3; x <= pos.x + 3; x++) {
            for (double z = pos.z - 3; z <= pos.z + 3; z++) {
                for (double y = pos.y + 5; y >= pos.y - 5; y--) {
                    if (canBePopulated(new LevelPosition(x, y, z, pos.level))) {
                        if (!canGrowPlant(new LevelPosition(x, y, z, pos.level)))
                            break;
                        double randomDouble = random.nextDouble();
                        if (randomDouble >= 0 && randomDouble < 0.3125) {
                            pos.level.setBlock(new LevelPosition(x, y, z, pos.level), Block.get(BlockID.TALL_GRASS), true, true);
                        }
                        if (randomDouble >= 0.3125 && randomDouble < 0.46875) {
                            pos.level.setBlock(new LevelPosition(x, y, z, pos.level), Block.get(BlockID.MOSS_CARPET), true, true);
                        }
                        if (randomDouble >= 0.46875 && randomDouble < 0.53125) {
                            if (canBePopulated2BlockAir(new LevelPosition(x, y, z, pos.level))) {
                                BlockLargeFern rootBlock = new BlockLargeFern();
                                rootBlock.setTopHalf(false);
                                pos.level.setBlock(new LevelPosition(x, y, z, pos.level), rootBlock, true, true);
                                BlockLargeFern topBlock = new BlockLargeFern();
                                topBlock.setTopHalf(true);
                                pos.level.setBlock(new LevelPosition(x, y + 1, z, pos.level), topBlock, true, true);
                            } else {
                                BlockTallGrass block = new BlockTallGrass();
                                pos.level.setBlock(new LevelPosition(x, y, z, pos.level), block, true, true);
                            }
                        }
                        if (randomDouble >= 0.53125 && randomDouble < 0.575) {
                            pos.level.setBlock(new LevelPosition(x, y, z, pos.level), Block.get(BlockID.AZALEA), true, true);
                        }
                        if (randomDouble >= 0.575 && randomDouble < 0.6) {
                            pos.level.setBlock(new LevelPosition(x, y, z, pos.level), Block.get(BlockID.FLOWERING_AZALEA), true, true);
                        }
                        if (randomDouble >= 0.6 && randomDouble < 1) {
                            pos.level.setBlock(new LevelPosition(x, y, z, pos.level), Block.get(BlockID.AIR), true, true);
                        }
                        break;
                    }
                }
    ...
    public boolean canGrowPlant(LevelPosition pos) {
